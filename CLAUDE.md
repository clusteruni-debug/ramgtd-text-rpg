> ⚠️ **이 프로젝트는 글로벌 규칙(~/.claude/CLAUDE.md)의 적용을 받습니다.**
> 글로벌 규칙 요약: 성급한 진단 금지(원인 3개 나열→검증→수정) | 구현 후 검증 필수(빌드/실행 확인 전 성공 보고 금지) | git push 전 git status 필수 | 민감 데이터 하드코딩 금지 | 요청 scope 전부 처리
